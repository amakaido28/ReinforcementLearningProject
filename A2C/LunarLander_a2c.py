...
def main():
    ...
    if(load_weights):
        #actor load model
        actor = Actor(state_num,action_num)
        adam_actor = torch.optim.Adam(actor.parameters(), lr=2e-3)
        ...
        #critic load model
        critic = Critic(state_num)
        adam_critic = torch.optim.Adam(critic.parameters(), lr=2e-3)
    ...
    for i in range(total_episode):
        ...
        while True:
            ...
            if done:
                ...
                x=round(np.mean(ep_rewards[-60:]), 3)
                if(x>best_score):
                    best_score=round(np.mean(ep_rewards[-60:]), 3)
                    torch.save(
                        {'step': i,
                ...
                break
        media=round(np.mean(ep_rewards[-60:]), 3)
        if media>=220:
